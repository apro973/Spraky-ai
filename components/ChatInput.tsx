...
const ChatInput = ({ onSend, disabled }) => {
  ...
  useEffect(() => {
    // Fix: Use type casting to 'any' to access SpeechRecognition properties on window to resolve TS errors.
  ...
  // Fix: Added optional event parameter to resolve the 'Expected 1 arguments, but got 0' error.
  const handleSubmit = (e?: React.FormEvent | React.KeyboardEvent) => {
    e?.preventDefault();
    if (inputValue.trim() && !disabled) {
      onSend(inputValue);
      setInputValue('');
      if (isRecording && recognition) (recognition as any).stop();
    }
  };
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter' && !e.shiftKey) {
      e.preventDefault();
      // Fix: Pass the current event to handleSubmit to maintain correct argument counts.
      handleSubmit(e);
    }
  };
